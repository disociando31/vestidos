...
<div class="card-body">
    ...
    @if(count($trajesCaballero))
        ...
        <div class="table-responsive">
            <table class="table table-bordered border-primary">
                ...
                <tbody>
                    @foreach($trajesCaballero as $pieza)
                        <tr>
                            <td>{{ $pieza['nombre'] ?? '-' }}</td>
                            <td>{{ $pieza['color'] ?? '-' }}</td>
                            <td>{{ $pieza['talla'] ?? '-' }}</td>
    ...
    @if(count($trajesNino))
        <h6 class="mt-4 text-info">Traje de Niño</h6>
        <div class="table-responsive">
            <table class="table table-bordered border-info">
                ...
                <tbody>
                    @foreach($trajesNino as $traje)
                        <tr>
                            <td>{{ $traje['nombre'] ?? '-' }}</td>
                            <td>{{ $traje['color'] ?? '-' }}</td>
                            <td>{{ $traje['talla'] ?? '-' }}</td>
                            <td>
                                @if(isset($traje['precio']))
                                    ${{ number_format((float)$traje['precio'], 2) }}
                                @else
                                    -
                                @endif
                            </td>
    ...
    @if(count($otrosAdicionales))
        <h6 class="mt-4">Adicionales</h6>
        <div class="table-responsive">
            <table class="table table-bordered">
                <thead class="table-light">
                    <tr>
                ...
                <tbody>
                    @foreach($otrosAdicionales as $adicional)
                        <tr>
                            <td>{{ $adicional['nombre'] ?? '-' }}</td>
                            <td>{{ $adicional['color'] ?? '-' }}</td>
                            <td>{{ $adicional['talla'] ?? '-' }}</td>
                            <td>
                                @if(isset($adicional['precio']))
                                    ${{ number_format((float)$adicional['precio'], 2) }}
                                @else
                                    -
                                @endif
                            </td>
